...
/
 ...
 -- 문제 레퍼런스 타입의 변수로 EID, ENAME, JCODE, DTITLE, SAL를 선언하고 각 변수의 자료형은 EMPLOYEE 테이블의 EMP_ID, EMP_NAME, JOB_CODE, SALARY 
 -- 컬럼과 DEPARTMENT 테이블의 DEPT_TITLE 컬럼의 자료형을 참조 사용자가 입력한 사번과 일치하는 사원을 조회한 후 조회 결과를 출력
 ...
 */
 ...
 -- 단, 보너스를 받지 않은 사원은 보너스 출력 전에 '보너스를 지급받지 않는 사원입니다.'라는 문구를 출력
 DECLARE
      EID EMPLOYEE.EMP_ID%TYPE;
      ENAME EMPLOYEE.EMP_NAME%TYPE;
      SAL EMPLOYEE.SALARY%TYPE;
 ...
 */
 ...
 -- 사용자에게 점수를 입력받아 SCORE 변수(데이터 타입 : NUMBER)에 저장한 후 학점은 입력된 점수에 따라 GRADE 변수(데이터타입 : CHAR(1))에 저장 90점 이상은 'A', 80점 이상은 'B', 70점 이상은 'C', 60점 이상은 'D', 60점 미만은 'F'
 ...
 */
 ...
 -- DEPT_CODE에 따라서 알맞는 부서를 출력(DNAME 데이터타입은 VARCHAR2(30))
 -- D1인 경우 인사관리부, D2인 경우 회계관리부, D3인 경우 마케팅부, D4인 경우 국내영업부, D5인 경우 해외영업1부, D6인 경우 해외영업2부, D7인 경우 해외영업3부, D8인 경우 기술지원부, D9인 경우 총무부, 나머지는 부서없음
  ...
  */
     ...
     CASE EMP.DEPT_CODE
     WHEN 'D1' THEN DNAME := '인사관리부';
     WHEN 'D2' THEN DNAME := '회계관리부';
     WHEN 'D3' THEN DNAME := '마케팅부';
     WHEN 'D4' THEN DNAME := '국내영업부';
     WHEN 'D5' THEN DNAME := '해외영업1부';
     WHEN 'D6' THEN DNAME := '해외영업2부';
     WHEN 'D7' THEN DNAME := '해외영업3부';
     WHEN 'D8' THEN DNAME := '기술지원부';
     WHEN 'D9' THEN DNAME := '총무부';
     ELSE DNAME := '부서없음';    
     END CASE;
 ...
 */
 ...
 BEGIN 
    ...
    WHILE NUM1 <= 9  
    LOOP
    DBMS_OUTPUT.PUT_LINE(NUM||' X '||NUM1||' = '||NUM*NUM1);
       NUM1:= NUM1 +1;  
       END LOOP; 
       NUM1 :=1;
       NUM := NUM + 1;
...
/ 
 ...
 /*
     ...
     * 오라클에서 미리 정의되어 있는 시스템 예외
     ...
     - TOO_MANY_ROWS : SELECT한 결과가 한행이 리턴되어야 하는데 여러 행일 경우
