 /*
     *DDL(Data Definition Language) : 데이터 정의어
     - 오라클에서 제공하는 객체(object)/스키마(Schema)를 만들고(CREATE), 변경(ALTER), 삭제(DROP)하는 언어
     - 즉, 실제 데이터 값이 아닌 구조 자체를 정의하는 언어
     - 주로 DB 관리자, 설계자가 사용
     
     
     * 오라클에서 객체(구조) : 테이블(TABEL), 뷰(VIEW), 시퀀스(SEQUENCE), 인덱스(INDEX), 패키지(PACKAGE), 트리거(TRIGGER), 프로시저(PROCEDURE), 함수(FUNCTION), 동의어(SYNONYM), 사용자(USER)
 */
 ...
 /*
     ...
     * 테이블 생성
     - 테이블이란? 행(ROW)과 열(COLUMN)로 구성되는 가장 기본적인 데이터베이스 객체, 데이터베이스 내에 모든 데이터는 테이블에 저장!
 ...
 */
 ...
 /*
     * 제약조건(CONSTRAINTS)
     ...
     - 종류 : NOT NULL, UNIQUE, CHECK, PRIMARY KEY, FOREIGE KEY
 ...
 */
 ...
 INSERT INTO MEM_UNIQUE VALUES(1,'user01','pass02','강개똥',null,null,null);
 ...
 */
 ...
 CREATE TABLE MEM_UNIQUE(
     ...
     MEM_NAME VARCHAR2(20) CONSTRAINT MENRENAME_NN NOT NULL,
     GENDER CHAR(3),
     PHONE VARCHAR2(13),
     EMAIL VARCHAR2(50),
...
-------------------------------- 실습문제 --------------------------------------------------------
 ...
 CREATE TABLE TB_PUBLISHER(
          ...
          PHONE VARCHAR2(13),
          CONSTRAINT TB_PUBLISHER_PUB_NO_PK PRIMARY KEY(PUB_NO)                      --  키같은경우 보통 마지막에 작성함
 );
...
INSERT INTO TB_BOOK VALUES(4,'혼자 공부하는 얄팍한 고딩 지식','고현민',16200,3); 
INSERT INTO TB_BOOK VALUES(5,'자라기','김창준',11700,1); 
...
-- 대여 정보 테이블
 ...
 -- CONSTRAINT TB_RENT_RENT_MEM_NO_PK FOREIGN KEY(RENT_MEM_NO) REFERENCES TB_MEMBER ON DELETTE SET NULL,
 -- CONSTRAINT TB_RENT_RENT_BOOK_NO_PK FOREIGN KEY(RENT_BOOK_NO) REFERENCES TB_BOOK ON DELETTE SET NULL,
 ...
 SELECT  MEMBER_NAME , MEMBER_ID,RENT_BOOK_NO,RENT_DATE, RENT_DATE+7 "반납예정일"
 FROM TB_MEMBER M
