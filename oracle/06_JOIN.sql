 ...
 /*
    ...
    - 관계형 데이터베이스는 최소한의 데이터로 각각의 테이블에 담고 있음 (중복을 최소화하기 위해 최대한 쪼개서 관리)
      ...
      - 크게 "오라클 구문"과 "ANSI 구문"
      - ANSI(미국국립표준협회 == 산업표준을 제정하는 단체) : 다른 DBMS에서도
 */

/*
   1. 오라클 - 등가 조인(EQUAL JOIN) / ANSI - 내부 조인(INNER JOIN / NATURAL JOIN) - 연결시키는 컬럼의 값이 일치하는 행들만 조인되서 조회 (일치하는 값이 없는 행은 조회X)
...
-- >> ANSI 구문
-- 해결방법 1) USING 이용하는 방법
SELECT EMP_ID, EMP_NAME, JOB_CODE, JOB_NAME
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE);

-- 해결방법 2) 테이블명을 이용하는 방법
SELECT EMP_ID, EMP_NAME, JOB.JOB_CODE, JOB_NAME
FROM EMPLOYEE 
JOIN JOB  ON(EMPLOYEE.JOB_CODE = JOB.JOB_CODE);

-- 해결방법 3) 테이블에 별칭을 부여해서 이용하는 방법
SELECT EMP_ID, EMP_NAME, J.JOB_CODE, JOB_NAME
FROM EMPLOYEE E
...
SELECT EMP_NAME , DEPT_TITLE , SALARY, SALARY*12
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID(+);  -- 기준으로 삼고자하는 테이블의 반대편 테이블의 컬럼 뒤에 (+)를 붙이기
...
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
WHERE SUBSTR(EMP_NO,1,2) >= 70                   -- EMP_NO LIKE '7%'      7로 시작
AND SUBSTR(EMP_NO,8,1) = '2' 
AND SUBSTR(EMP_NAME,1,1) = '전';                 -- EMP_NAME LIKE '전%'    전으로 시작
...
WHERE E.DEPT_CODE = D.DEPT_ID 
AND E.JOB_CODE = J.JOB_CODE 
AND SUBSTR(EMP_NO,1,2) >= 70                 -- EMP_NO LIKE '7%'      7로 시작
AND SUBSTR(EMP_NO,8,1) = '2' 
AND SUBSTR(EMP_NAME,1,1) = '전';             -- EMP_NAME LIKE '전%'    전으로 시작


-- 3. 보너스를 받는 직원들의 직원명, 보너스, 연봉, 부서명, 근무지역을 조회 단, 부서 코드가 없는 사원도 출력될 수 있게!
...
-- 5. 각 부서별 평균 급여를 조회하여 부서명, 평균 급여(정수 처리)를 조회 단, 부서 배치가 안된 사원들의 평균도 같이 나오게끔!
SELECT NVL(DEPT_TITLE,'부서없음') 부서명, TO_CHAR (ROUND(AVG(NVL(SALARY,0))),'999,999,999') 급여평균
FROM EMPLOYEE E
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY DEPT_TITLE
...
SELECT DEPT_TITLE 부서명, TO_CHAR (SUM(SALARY),'999,999,999') "급여의 합"
FROM EMPLOYEE E 
LEFT JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
GROUP BY DEPT_TITLE
...
-- 7. 사번, 직원명, 직급명, 급여 등급, 구분을 조회 이때 급여 등급이 S1, S2인 경우 '고급', 급여 등급이 S3, S4인 경우 '중급, 급여 등급이 S5, S6인 경우 '초급
SELECT EMP_ID 사번, EMP_NAME 직원명, JOB_NAME 직급명 , SAL_LEVEL "급여 등급", DECODE(SAL_LEVEL,'S1','고급','S2','고급','S3','중급','S4','중급','초급') 구분   -- CASE WHEN SAL_LEVEL IN('S1','S2') THEN '고급'
FROM EMPLOYEE E                                                                                                                                             -- WHEN SAL_LEVEL IN('S3','S4') THEN '중급'     
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)                                                                                                                      -- WHEN SAL_LEVEL IN('S5','S6') THEN '초급'
JOIN SAL_GRADE S ON (E.SALARY BETWEEN S.MIN_SAL AND S.MAX_SAL);                                                                                        -- END "구분"
...
SELECT EMP_NAME 직원명, JOB_NAME 직급명 , DEPT_TITLE 부서명, LOCAL_NAME "근무 지역"
FROM EMPLOYEE E
JOIN DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN JOB J ON(E.JOB_CODE = J.JOB_CODE)
JOIN LOCATION L ON(D.LOCATION_ID = L.LOCAL_CODE);
