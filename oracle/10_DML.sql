 /*
    * DML(Data Manipulation Language) : 데이터 조작 언어로 테이블에 값을 삽입(INSERT)하거나, 수정(UPDATE)하거나, 삭제(DELETE)하는 구문이다.
 ...
 */
 ...
 -- 표현법 1)
 CREATE TABLE EMP_DEPT AS SELECT EMP_ID, EMP_NAME, DEPT_CODE, HIRE_DATE FROM EMPLOYEE
 WHERE 1=0;
 ...
 --EMP_DEPT 테이블에 EMPLOYEE 테이블의 부서 코드가 D!인 지원의 사번, 이름, 부서코드, 입사일을 삽입하고
 --EMP_MANAGER 테이블에 EMPLOYEE 테이블의 부서코드가 D!인 직원의 사번,이름,관리자 사번을 조회하여 삽입한다.
...
SELECT  EMP_ID
FROM EMP_SALARY S
JOIN DEPT_COPY C ON (S.DEPT_CODE = C.DEPT_ID)
 ...
 */
 ...
 DELETE FROM EMP_SALARY  WHERE EMP_NAME = '선동일';
 ...
 -- DEPARTMENT 테이블에서 DEPT_ID D1인 부서 삭제 (여기도 다시!)
 DELETE DEPARTMENT WHERE DEPT_ID = 'D1';
