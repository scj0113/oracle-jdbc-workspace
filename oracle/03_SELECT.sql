 ...
 - 데이터(data) : 필요에 의해 수집했지만 아직 특정 목적을 위해 정제하지 않은 값
  ...
  - 관계형(Relational) 데이터베이스
   ...
   2. 데이터를 테이블 형태로 저장하고 여러 테이블을 조합하여 비즈니스 관계를 도충하는 구조
   3. 데이터의 중복을 최소화할 수 있으며 사용하기 편리하고 데이터의 무결성, 트랜잭션 처리 등 데이터 베이스 관리 시스템으로 뛰어난 성능을 보여준다.
  ...
  3. DQL (Date Query Language) : 데이터 질의어 - 데이터를 조회(검색)하기 위해 사용하는 언어 (SELECT)
  
  4. DCL (Data Control Language) : 데이터 제어어 - DBMS에 대한 보안, 무결성, 복구 등 DBMS를 제어하기 위한 언어 (GRANT 권한 부여, REVOKE 권한 회수)
 ...
 */
 ...
 -- EMPLOYEE 테이블에서 직원명(EMP_NAME), 보너스가 포함된 연봉(급여 + (보너스 * 급여)) * 12) 조회
 ...
 */
 ...
 --- EMPLOYEE에 직급코드 조회 (중복된값 제거)
 SELECT DISTINCT JOB_CODE  
 FROM EMPLOYEE;
 
 
 ---- EMPLOYEE에 부서코드 조회 (중복된값 제거)
 ...
 */
 ...
 -- 급여가 350만원이상 600만원 이하를 받는 사원들의 사원명, 사번, 급여
...
--- 직급코드가 J7이거나 J2인 사원들 중 급여가 200만원 이상인 사원들의 모든 컬럼 조회
 ...
 -- 5. 보너스 포함 연봉이 NULL이 아니고 이름에 '하'가 포함되어 있는 사원들의 사번, 사월명, 급여, 보너스포함연봉 조회 (별칭)
 ...
 */
 ...
 -- 전 사원의 사원명, 보너스 조회 (보너스 가준오름차순 정렬)
