  ...
  */
  ...
  /*
     ...
     * VIEW 옵션들
     ...
     - NOFORCE : 서비 쿼리에 기술된 테이블이 존재해야만 뷰가 생성된다. (기본값)
...
SELECT * FROM VW_EMP_JOB;
  ...
  -- 2) 뷰 생성 시 모든 컬럼에 별칭을 부여하는 방법 (모든컬럼에 별칭부여해야함)
...
/*
   VIEW 를 이용해서 DML(INSERT, UPDATE, DELETE) 사용 가능
...
/*
   ...
   2. 뷰에 포함되지 않은 컬럼 중에 베이스가 되는 컬럼이 NOTNULL 제약조건이 지정된 경우
...
INSERT INTO VW_EMP_SAL VALUES(400,'홍길동',3000000,360000000); -- 에러발생!
...
UPDATE VW_EMP_SAL
SET 연봉 = 80000000
WHERE EMP_ID = 200;  -- 에러발생 ( 산술연산으로 정의된 컬럼은 데이터 변경 불가능 )
...
-- INSERT (에러)
INSERT INTO VW_GROUPDEPT VALUES('D3',8000000,4000000); -- 에러발생
...
WHERE EMP_ID = 200;     -- 성공
 ...
 WHERE DEPT_TITLE = '총무부';   --성공
  ...
  -- 테이블 생성 후 만들기 가능 테이블 없을 시 에러 발생
  ...
  CREATE OR REPLACE FORCE VIEW VW_EMP AS SELECT * FROM TEST;
  ...
  CREATE OR REPLACE VIEW VW_DEPT AS SELECT * FROM DEPARTMENT
  WITH READ ONLY;
