 ...
 */
 ...
 -- 사원이 위와 같은 직원들의 사번, 이름, 부서 코드, 구분(사수) 조회
...
/*
  RANK 함수
  - RANK() OVER(정렬 기준) : 동일한 순위 이후의 등수를 동일한 인원수만큼 건너뛰로 순위를 계산 (ex. 공동 1위가 2명이면 다음 순위는 3위)
  - DENSE_RANK() OVER(정렬 기준) : 동일한 순위 이후의 등수를 무조건 1씩증가한다. (ex. 공동 1위가 2명이면 다음 순위는 2위)
*/
 -- 사원별 급여가 높은 순서대로 순위 매겨서 순위, 사원명, 급여 조회
 -- 공동 19위가 2명 뒤에 순위는 21위
