 ...
 */
 ...
 SELECT INSTR('AABAACAABBAA','B',-1) FROM DUAL;     -- 뒤에서부터 B찾기 순서조회는 앞에서부터 조회라 10 
 
 SELECT INSTR('AABAACAABBAA','B', 1, 2) FROM DUAL;  -- 앞에서부터 2번째 B 찾기
...
*/
 ...
 SELECT TRIM(BOTH 'Z' FROM 'ZZZKHZZZ') FROM DUAL;       -- 양쪽 다 제거
 ...
 */
 ...
 SELECT EMP_NAME, EMAIL, REPLACE (EMAIL,'kh.or.kr','gmail.com') "변경된 EMAIL"
 FROM EMPLOYEE;
...
*/
 ...
 SELECT EMP_NAME, HIRE_DATE, ADD_MONTHS(HIRE_DATE, 6)  "입사 후 6개월이 된 날짜"
 FROM EMPLOYEE;
 ...
 */
 ...
 -- 현재 언어가 KOREAN이기 때문에 : 에러발생  (언어변경시 다시 작동)
 ...
 */
 ...
 SELECT TO_CHAR(1234,'L99') FROM DUAL;      -- 포맷 자리수가 안맞아서! #으로 표시
 SELECT TO_CHAR(1234,'L999,999') FROM DUAL;
 
 -- 직원명, 급여, 연봉 (위의 TO_CHAR를 이용해서) 조회
 SELECT EMP_NAME, TO_CHAR(SALARY,'L999,999,999')||'원' 급여, TO_CHAR(SALARY*12,'L999,999,999')||'원' 연봉
 FROM EMPLOYEE
 ORDER BY 연봉;
 ...
 */
 ...
 SELECT TO_DATE(20230621153410) FROM DUAL;  
 ...
 */
 ...
 -- 직급 코드가 J7인 사원은 급여율 10% 인상
 -- 직급 코드가 j6인 사원은 급여를 15% 인상
 -- 직급 코드가 j5인 사원은 급여를 20% 인상
 -- 그 외의 직급의 사원은 급여를 5%만 인상
 -- 정렬 : 직급 코드 j1 부터, 인상된 급여 높은 순서대로
 
 SELECT EMP_NAME, JOB_CODE, SALARY , DECODE( JOB_CODE ,'J7', SALARY*1.1,'J6',SALARY*1.15,'J5',SALARY*1.2, SALARY*1.05) "인상된 급여"
 FROM EMPLOYEE
...
FROM EMPLOYEE;
 ...
 -- 급여값이 500만원 이하 350만원 초과 2등급
 -- 급여값이 350만원 이하 200만원 초과 3등급
 -- 그 외의 경우 4등급
