 ...
 */
 ...
 SELECT NVL(DEPT_CODE,'부서없음'), COUNT(*) "사원의 수", COUNT(BONUS) "보너스를 받는 사원의 수", SUM(SALARY) "총급여" , FLOOR(AVG(NVL(SALARY,0))) "평균 급여", MIN(SALARY) "최저 급여", MAX(SALARY) "최고 급여"
 FROM EMPLOYEE
 GROUP BY DEPT_CODE
 ...
 */
 ...
 -- 직급별 총 급여의 함이 1000만원 이상인 직급만 조회
...
-- 2. UNION ALL
 ...
 -- 3. INTERSECT
 -- 부서 코드가 D5이면서 급여가 300만원 초과인 사원들의 사번, 사원명, 부서 코드, 급여를 조회
 SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
 FROM EMPLOYEE
 WHERE DEPT_CODE = 'D5'
 INTERSECT
